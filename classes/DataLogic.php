...
class DataLogic {
    ...
    public static function uploadFile($fileData) {
        ...
        $hasUpload = move_uploaded_file($file_tmp_name, $local_path);
    ...
    public static function insertFile($fileData) {
        ...
        $sql = "
            INSERT INTO 
                images (user_id, post_id, use_for, file_name, local_path, remote_path, description)
            VALUES
                (?, ?, ?, ?, ?, ?, ?);
        ";
        ...
        $arr[] = $fileData["use_for"];
        $arr[] = $fileData["save_file_name"];
        $arr[] = $fileData["local_path"];
        $arr[] = $fileData["remote_path"];
    ...
    public static function showPostLimit($offset, $count, $user_id, $mode) {
        ...
        switch($mode) {
            case 1:
                $sql = "SELECT * FROM posts ORDER BY post_id DESC LIMIT :offset, :count";
                break;
            case 2:
                $sql = "SELECT * FROM posts WHERE user_id = :user_id ORDER BY post_id DESC LIMIT :offset, :count";
                break;
        }
    ...
    public static function getDescriptionByFilePath($path) {
        ...
        $sql = "SELECT * FROM images WHERE remote_path = ?;";
